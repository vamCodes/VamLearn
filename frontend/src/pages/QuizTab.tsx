...
import { generateQuizFromText } from '../utils/hf';
...
const QuizTab: React.FC<QuizTabProps> = ({ selectedPdf, extractedText }) => {
 ...
 const generateQuiz = async () => {
  ...
  try {
    ...
    const response = await generateQuizFromText(firstChunk);
    const rawText =response.choices?.[0]?.message?.content?.trim() ??  '';
   console.log("📘 Raw quiz text:", rawText);

// ✅ Extract the text content from Groq’s structured response


    // Parse questions from response.choices[0].message.content if needed
    ...
    if (response && Array.isArray(response.choices) && response.choices.length > 0) {
      try {
        ...
        console.error('Failed to parse quiz questions from response:', e);
      }
    }
